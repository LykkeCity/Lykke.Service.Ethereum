...
namespace Lykke.Service.Ethereum.Domain.Services
{
    ...
    public class BlockchainService : BlockchainServiceBase, IBlockchainService
    {
            ...
            string signedTxData)
        {
            ...
            var txReceipt = await SendRequestWithTelemetryAsync<Nethereum.RPC.Eth.DTOs.TransactionReceipt>
            (
        ...
        private async Task<bool> CheckIfBroadcastedAsync(
            string txHash)
        {
            var transaction = await SendRequestWithTelemetryAsync<Nethereum.RPC.Eth.DTOs.Transaction>
            (
